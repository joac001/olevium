  ...
  useEffect(() => {
    if (!categories.length) {
      fetchCategories().catch((error) => {
        const message = error instanceof Error ? error.message : "No pudimos obtener las categorías.";
        showNotification("fa-solid fa-triangle-exclamation", "danger", "Error al cargar categorías", message);
      });
    }
  ...
  useEffect(() => {
    if (!hasFetchedUser) {
      fetchCurrentUser().catch((error) => {
        const message = error instanceof Error ? error.message : "No pudimos obtener la información de usuario.";
        showNotification("fa-solid fa-triangle-exclamation", "danger", "Error al cargar usuario", message);
      });
    }
              ...
              return (
                ...
                >
                  <Box className="flex flex-1 flex-col gap-1">
                    ...
                    <Typography variant="caption" className="text-xs uppercase tracking-[0.18em] text-[color:var(--text-muted)]">
                      Tipo: <span className="font-semibold text-[color:var(--text-primary)]">{typeLabel}</span>
                    </Typography>
                    {category.color && (
                      <Box className="flex items-center gap-2 w-fit">
                        <span className="h-4 w-4 rounded-xl" style={{ backgroundColor: category.color }}></span>
