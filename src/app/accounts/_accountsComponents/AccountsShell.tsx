...
export default function AccountsShell() {
  ...
  useEffect(() => {
    fetchAccounts().catch((error) => {
      ...
      showNotification("fa-solid fa-triangle-exclamation", "danger", "Error al cargar cuentas", message);
    });

    fetchAccountTypes().catch((error) => {
      const message = error instanceof Error ? error.message : "No pudimos cargar los tipos de cuentas.";
      ...
      <Card tone="neutral" title="Tus cuentas" className="h-full">
        <Box className="space-y-4">
          <Box className="space-y-3 rounded-2xl bg-[color:var(--surface-accent)] p-4 text-left shadow-sm">
            <Typography variant="body" className="text-sm uppercase tracking-[0.22em] text-[color:var(--text-primary)]">
              Balances
            </Typography>
