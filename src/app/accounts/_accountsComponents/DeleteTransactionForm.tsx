...
export default function DeleteTransactionForm({ transaction, currency, onSuccess }: DeleteTransactionFormProps) {
  ...
  const handleSubmit = useCallback(
    async (formData: FormData) => {
      ...
      try {
        ...
        showNotification("fa-solid fa-circle-check", "success", "Transacción eliminada", "Quitamos el movimiento de la cuenta.");
        onSuccess?.();
      } catch (error) {
        const message = error instanceof Error ? error.message : "No se pudo eliminar la transacción.";
        showNotification("fa-solid fa-triangle-exclamation", "danger", "Error al eliminar", message);
      } finally {
  ...
  const amountLabel = useMemo(() => formatAmount(transaction.amount, currency), [transaction.amount, currency]);
      ...
      <Box className="space-y-3">
        ...
        <Box className="rounded-2xl border border-[color:var(--surface-muted)] p-4">
          <Typography variant="body" className="text-sm text-[color:var(--text-primary)]">
            {transaction.description && transaction.description.trim().length ? transaction.description : "Sin descripción"}
          </Typography>
          <Typography variant="body" className="text-sm text-[color:var(--text-muted)]">
            Fecha: <span className="font-semibold text-[color:var(--text-primary)]">{dateLabel}</span>
          </Typography>
          <Typography variant="body" className="text-sm text-[color:var(--text-muted)]">
