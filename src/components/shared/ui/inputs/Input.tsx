...
const Input = forwardRef<InputRef, InputProps>(
  (
    ...
    const syncInputValue = useCallback(
      (rawValue: string | number, localType: InputType = inputType) => {
        const processed = localType === 'date' ? normalizeDateDisplayValue(rawValue) : rawValue;
    ...
    const applyValue = useCallback(
      (rawValue: string | number) => {
        const processed = inputType === 'date' ? formatDateInput(String(rawValue)) : rawValue;
        setInputValue(processed);
        syncValidation(processed);
    ...
    const openDatePicker = () => {
      ...
      if (pickerInput) {
        ...
        const showPicker = (pickerInput as HTMLInputElement & { showPicker?: () => void })
          .showPicker;
